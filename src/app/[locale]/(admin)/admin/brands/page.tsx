      ...
      <WhiteCard noPadding headerAction={
        <div className="flex gap-2">
          <div className="relative">
            <input
              ...
              className="pl-8 pr-3 py-1.5 border border-gray-300 rounded text-sm focus:outline-none focus:border-blue-500 w-64"
            />
        ...
        <table className="w-full text-left text-sm text-gray-600">
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              <tr><td colSpan={5} className="px-5 py-10 text-center">Loading brands...</td></tr>
            ) : brands.length === 0 ? (
              <tr><td colSpan={5} className="px-5 py-10 text-center">No brands found.</td></tr>
            ) : (
              brands.map((brand: any) => (
                <tr key={brand._id || brand.id} className="hover:bg-gray-50/50 transition-colors group">
                  ...
                  <td className="px-5 py-3 font-medium text-gray-800">{brand.name?.en || brand.name}</td>
                  <td className="px-5 py-3 font-medium text-gray-800">{brand.name?.ar || "-"}</td>
